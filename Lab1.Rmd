...
```{r read.data, warning = F, echo = T, message = F}
library(tidyverse)
...
#### 2. 
Use the `group_by()` function along with `summarize()` to calculate the mean `IgG S/CO` value for the two groups (symptomatic and asymptomatic) in the dataset.

#### 3.
Using `ggplot2` create a figure that compares the `IgG S/CO` value for the symptomatic and asymptomatic groups.
